  ...
  useEffect(() => {
    ...
    const renderFrame = (timestamp: number) => {
      ...
      if (sp < 0.85) {
        ...
        // Post-Cave Exit Daylight Angle (sp >= 0.85): Vehicle swap to Wheel Loader synchronized with camera angle change!
        truckGroup.visible = false;
        if (loaderGroup) {
          loaderGroup.visible = true;
          loaderGroup.position.copy(pathTransform.position);
          loaderGroup.rotation.set(0, 0, 0); // Container rotation reset so inner model rotation governs facing direction
        }
      }

      // Sprinkle glowing gold ore particles directly from loader bucket scoop (sp >= 0.85)
      if (sp >= 0.85) {
        bucketScoopPos.copy(pathTransform.position).add(new THREE.Vector3(3.0, 0.55, 0));
      ...
      if (camFrame.up) {
        camera.up.lerp(camFrame.up, Math.min(delta * 4, 1));
      }
