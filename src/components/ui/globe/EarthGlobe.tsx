...
export const EarthGlobe: React.FC<EarthGlobeProps> = ({
  ...
  useEffect(() => {
    ...
    // --- Digital Telemetry Orbital Rings ----------------------------------------
    const ringGroup = new THREE.Group();
    const createOrbitalRing = (radius: number, yOffset: number, colorHex: string, opacity: number) => {
      const segments = 96;
      const pts: THREE.Vector3[] = [];
      for (let i = 0; i <= segments; i++) {
        const theta = (i / segments) * Math.PI * 2;
        pts.push(new THREE.Vector3(Math.cos(theta) * radius, yOffset, Math.sin(theta) * radius));
      }
    ...
    // Equator ring
    ringGroup.add(createOrbitalRing(1.026, 0, "#D4AF37", 0.40));
    // Tropic of Cancer
    ringGroup.add(createOrbitalRing(1.022, 0.40, "#7EB6FF", 0.22));
    // Tropic of Capricorn
    ringGroup.add(createOrbitalRing(1.022, -0.40, "#7EB6FF", 0.22));
    earth.add(ringGroup);
