        ...
        <div className="mt-14 md:mt-20">
          {PRINCIPLES.map((principle, index) => (
            <div
              ...
              className="group relative border-t border-[#E5E4DE] last:border-b transition-colors duration-300 hover:bg-[#FAF5E8]/40 cursor-default"
            >
              {/*
                The hover rule, sitting on the row's own top border.
              */}
              <span
                aria-hidden="true"
                className="pointer-events-none absolute inset-x-0 -top-px h-[2px] origin-left scale-x-0 bg-[#B8860B] transition-transform duration-500 ease-out group-hover:scale-x-100"
              />

              <div className="grid grid-cols-1 items-baseline gap-x-16 gap-y-4 py-8 px-4 md:py-10 md:px-6 lg:grid-cols-12">
                <div className="flex items-baseline gap-5 lg:col-span-5">
                  <span
                    data-about-reveal
                    className={`font-mono text-[11px] tabular-nums text-[#B8860B] transition-transform duration-300 group-hover:scale-110 ${HIDDEN_RISE}`}
                  >
                    {String(index + 1).padStart(2, "0")}
                  </span>

                  <h3 className="font-serif text-[clamp(2rem,4.6vw,3.5rem)] font-normal leading-[1.05] tracking-[-0.025em] text-[#0B1F3A] transition-all duration-300 group-hover:translate-x-1 group-hover:text-[#B8860B]">
                ...
                <p
                  data-about-reveal
                  className={`max-w-[44ch] text-lg font-normal leading-relaxed text-[#57595E] transition-colors duration-300 group-hover:text-[#1A1D21] sm:text-xl lg:col-span-7 ${HIDDEN_RISE}`}
                >
